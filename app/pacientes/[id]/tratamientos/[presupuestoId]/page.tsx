  ...
  const guardarAbono = async () => {
    ...
    try {
      ...
      await supabase.from('pagos').insert([{
        ...
        caja_id: cajasOpen[0].id, numero_boleta: numBoleta, numero_referencia: numReferencia, fecha_pago: new Date().toISOString()
      }]);
        ...
        {dienteSeleccionado && (
          ...
          >
            <div className="pt-20 pb-8 px-10 bg-slate-900 text-white text-left relative">
              ...
              <h3 className="text-2xl font-black uppercase italic mt-4 text-left">Nuevo Tratamiento</h3>
            </div>
                          ...
                          {filtrados.map(p => (
                            <button key={p.id} onClick={() => handleSeleccionarTratamiento(p)} className="w-full text-left p-4 hover:bg-white rounded-2xl transition-all group flex justify-between items-center">
                               <div className="max-w-[70%] text-left">
                                 <p className="text-[10px] font-black uppercase text-slate-700 group-hover:text-blue-600 leading-tight text-left">{p["Nombre Accion"]}</p>
                                 <p className="text-[9px] font-bold text-emerald-600 mt-1 text-left">${Number(p["Precio"]).toLocaleString('es-CL')}</p>
                               </div>
