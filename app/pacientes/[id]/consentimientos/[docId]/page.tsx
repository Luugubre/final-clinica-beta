...
export default function DetalleConsentimientoPage() {
  ...
  async function fetchDatos() {
    ...
    try {
      ...
      // 3. Obtener datos extendidos del especialista
      if (doc.especialista_id) {
        ...
        if (profRes.data) {
          ...
          setEspecialista({
            nombre_completo: `Dr/a. ${profData.nombre} ${profData.apellido}`,
            especialidad: profData.especialidades?.nombre || 'Especialista',
  ...
  const guardarFirmaEspecialista = async () => {
    if (!sigCanvas.current || sigCanvas.current.isEmpty()) {
        return toast.error("El especialista debe firmar antes de finalizar")
    }
  ...
  if (cargando) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50 gap-4">
      ...
      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Generando Documento Legal...</p>
    </div>
  )

  if (errorCritico) return <div className="p-20 text-center font-black text-red-500 uppercase">{errorCritico}</div>
    ...
    <div className="min-h-screen bg-slate-200 font-sans print:bg-white text-left">
      ...
      <nav className="sticky top-0 bg-white/90 backdrop-blur-md border-b border-slate-200 px-6 py-4 flex justify-between items-center z-50 print:hidden">
        <div className="flex items-center gap-4 text-left">
          ...
          <div className="text-left">
            <h2 className="text-[10px] font-black uppercase text-slate-400 tracking-widest leading-none mb-1 text-left">Documento Clínico</h2>
            <p className="text-sm font-bold text-slate-800 uppercase italic tracking-tighter text-left">{documento?.nombre_consentimiento}</p>
          </div>
        </div>
      ...
      <main className="w-full flex flex-col items-center p-4 md:p-12 print:p-0 print:block">
        ...
        <div id="hoja-imprimible" className="w-full max-w-[850px] bg-white shadow-2xl rounded-sm flex flex-col mx-auto print:shadow-none print:w-full text-left">
          ...
          <div className="p-10 sm:p-16 md:p-20 flex flex-col flex-grow print:p-8">
            
            {/* CABECERA */}
            <div className="flex justify-between items-start border-b-2 border-slate-900 pb-8 mb-10 text-left">
              <div className="flex items-start gap-4 text-left">
                ...
                <div className="mt-2 text-left">
                  <h1 className="text-lg font-black uppercase text-slate-900 leading-tight text-left">Centro Médico y Dental Dignidad</h1>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-8 mb-12 bg-slate-50 p-8 rounded-2xl border border-slate-100 print:bg-white print:border-slate-200 print:mb-8 text-left">
              <div className="space-y-1 text-left">
                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest text-left">Datos del Paciente</span>
                <p className="text-sm font-black text-slate-800 uppercase leading-none text-left">{paciente?.nombre} {paciente?.apellido}</p>
                <p className="text-xs text-slate-500 font-bold mt-1 text-left">RUT: {paciente?.rut || '---'}</p>
              </div>
              <div className="text-right space-y-1">
                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest text-right">Especialista Responsable</span>
                <p className="text-sm font-black text-slate-800 uppercase leading-none text-right">{especialista?.nombre_completo || documento?.creado_por}</p>
                <p className="text-xs text-slate-500 font-bold mt-1 text-right">{especialista?.especialidad} • RUT: {especialista?.rut}</p>
              </div>
            </div>
            ...
            <div className="relative flex-grow print:overflow-visible text-left">
              <div 
                className="prose prose-slate max-w-full text-slate-700 text-sm leading-[1.8] text-justify break-words print:text-black print:text-[11pt] text-left"
            ...
            <div className="mt-20 grid grid-cols-2 gap-20 pt-10 border-t-2 border-slate-100 print:mt-10 print:break-inside-avoid">
              ...
              <div className="text-center flex flex-col items-center">
                ...
                <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest text-center">Firma Especialista</p>
                <p className="text-[10px] font-bold text-slate-800 uppercase mt-2 text-center">{especialista?.nombre_completo || documento?.creado_por}</p>
                <p className="text-[8px] text-slate-400 uppercase text-center">{especialista?.especialidad}</p>
              </div>

              {/* FIRMA PACIENTE */}
              <div className="text-center flex flex-col items-center">
                <div className="w-full h-24 border-b border-slate-200 mb-4 flex items-center justify-center bg-slate-50/50 print:bg-transparent overflow-hidden">
                  {documento?.img_firma_paciente ? (
                    ...
                    <span className="text-[10px] text-slate-300 font-black uppercase italic text-center">Firma del Paciente</span>
                  )}
                </div>
                <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest text-center">Aceptación Paciente</p>
                <p className="text-[10px] font-bold text-slate-800 uppercase mt-2 italic text-center">{paciente?.nombre} {paciente?.apellido}</p>
                <p className="text-[8px] text-slate-400 uppercase text-center">RUT: {paciente?.rut}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {documento?.firma_profesional !== 'Firmado' && (
          <aside className="w-full max-w-[400px] mt-10 print:hidden mx-auto">
            <div className="bg-white p-10 rounded-[3rem] shadow-2xl border-2 border-slate-900 text-left">
              <h3 className="text-sm font-black uppercase italic text-slate-800 mb-6 flex items-center gap-2 text-left">
