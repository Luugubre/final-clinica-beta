  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500 text-left">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-800 uppercase italic tracking-tighter">Galería Multimedia</h2>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">Gestión de Radiografías y Documentos</p>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            ...
            <input 
              ...
              className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"
              value={filtro}
              onChange={(e) => setFiltro(e.target.value)}
            />
          </div>
          
          <label className="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-2xl font-black text-[10px] uppercase shadow-lg shadow-blue-200 transition-all flex items-center gap-2 shrink-0">
      ...
      {archivosFiltrados.length > 0 ? (
          ...
          <AnimatePresence>
            {archivosFiltrados.map((arc) => (
              <motion.div 
                ...
                key={arc.id}
                className="group bg-white rounded-[2rem] border border-slate-100 overflow-hidden shadow-sm hover:shadow-xl hover:border-blue-200 transition-all"
              >
                {/* PREVIEW */}
                <div className="aspect-video bg-slate-50 relative overflow-hidden flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3 backdrop-blur-sm">
                    ...
                    <a href={arc.url_archivo} target="_blank" download className="p-3 bg-white rounded-xl text-slate-900 hover:bg-blue-600 hover:text-white transition-all">
                      <Download size={18} />
                    </a>
                ...
                <div className="p-5 text-left">
                  <h3 className="text-xs font-black text-slate-800 uppercase truncate mb-1">{arc.titulo || 'Sin título'}</h3>
                  <div className="flex justify-between items-center">
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                      {new Date(arc.fecha_subida).toLocaleDateString('es-CL')}
                    </span>
                    <span className="text-[8px] px-2 py-0.5 bg-slate-100 rounded-md font-black text-slate-500 uppercase">
                      {arc.tipo_archivo.split('/')[1]}
                    </span>
                  </div>
                </div>
              </motion.div>
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-slate-50/50 rounded-[3rem] border-2 border-dashed border-slate-100">
          ...
          <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">No hay archivos registrados</h3>
          <p className="text-xs text-slate-300 mt-2">Sube radiografías, fotos o exámenes del paciente.</p>
        </div>
      )}
        ...
        {modalImagen && (
          ...
          >
            ...
            >
              ...
              <img src={modalImagen} className="rounded-2xl shadow-2xl max-h-[85vh] object-contain" />
            </motion.div>
          </div>
        )}
      </AnimatePresence>
