...
export default function DocumentosPage() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // 1. OBTENER EL USUARIO ACTUAL (Para el historial)
      ...
      // 3. INSERCIÓN EN TABLA (Con profesional_id para el historial)
      const { data, error } = await supabase.from('documentos_pacientes').insert([{
        ...
        titulo: file.name,
        profesional_id: user.id // <-- CAMPO CLAVE PARA EL HISTORIAL
      }]).select().single()
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 pb-20 text-left">
      ...
      <div className="bg-white p-10 rounded-[3.5rem] shadow-sm border border-slate-100 flex justify-between items-center mb-10 text-left">
        ...
        <label className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase cursor-pointer hover:bg-slate-900 shadow-xl transition-all flex items-center gap-2 active:scale-95">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 text-left">
        {documentos.length === 0 ? (
          <div className="col-span-full py-24 text-center bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200">
             <p className="text-[10px] font-black uppercase text-slate-300 tracking-widest italic">Sin registros digitalizados</p>
          </div>
        ) : (
          documentos.map((doc) => (
            <motion.div 
            ...
            >
              <div className="aspect-square bg-slate-50 rounded-[2rem] mb-4 overflow-hidden flex items-center justify-center relative">
                {doc.tipo_archivo?.includes('image') ? (
                  <img src={doc.url_archivo} className="w-full h-full object-cover transition-transform group-hover:scale-110" />
                ) : (
      ...
      <AnimatePresence>
        {visorAbierto && (
          <motion.div 
            ...
            className="fixed inset-0 bg-slate-950/98 backdrop-blur-3xl z-[999] flex flex-col lg:flex-row overflow-hidden"
          >
            {/* IZQUIERDA: VISUALIZADOR */}
            <div className="lg:w-[80%] w-full h-full relative flex items-center justify-center p-4 lg:p-12 overflow-hidden">
               ...
               <div className="w-full h-full flex items-center justify-center overflow-hidden">
                  {seleccionado?.tipo_archivo?.includes('image') ? (
                    ...
                    >
                      <img src={seleccionado.url_archivo} className="max-w-full max-h-[85vh] object-contain rounded-sm select-none pointer-events-none shadow-2xl" draggable={false} />
                    </motion.div>
                  ) : (
                    <div className="flex flex-col items-center gap-6">
                       <FileText size={120} className="text-white/10" />
                       <a href={seleccionado.url_archivo} target="_blank" className="bg-white text-slate-900 px-10 py-5 rounded-3xl font-black text-xs uppercase shadow-2xl hover:scale-105 transition-transform">Abrir PDF en Nueva Pestaña</a>
                    </div>
                  )}
               </div>
            </div>
            ...
            <motion.div initial={{ x: 100 }} animate={{ x: 0 }} className="lg:w-[20%] w-full bg-white h-full p-10 flex flex-col shadow-2xl z-30 text-left">
              <div className="flex-1 space-y-8 overflow-y-auto pr-2 text-left">
                <div className="space-y-2 text-left">
                  <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest flex items-center gap-2"><FileType size={12}/> {seleccionado.tipo_archivo?.split('/')[1]}</p>
                ...
                <div className="pt-6 border-t border-slate-100 text-left">
                   <p className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-2"><Calendar size={14} /> {new Date(seleccionado.fecha_subida).toLocaleDateString()}</p>
                </div>
                <div className="space-y-6 pt-10 text-left">
                  <div className="space-y-3 text-left">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 italic">Título Visual</label>
                    <input type="text" className="w-full bg-slate-50 p-5 rounded-2xl text-xs font-bold outline-none border-2 border-transparent focus:border-blue-500/20" value={editando.titulo} onChange={(e) => setEditando({...editando, titulo: e.target.value})} />
                  </div>
                  <div className="space-y-3 text-left">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 italic">Descripción / Hallazgos</label>
                    <textarea className="w-full bg-slate-50 p-5 rounded-2xl text-xs font-bold outline-none border-2 border-transparent focus:border-blue-500/20 min-h-[180px]" value={editando.descripcion} onChange={(e) => setEditando({...editando, descripcion: e.target.value})} />
                  </div>
                </div>
              </div>
