...
import { 
  ...
  Banknote, CreditCard, Landmark, User, ShieldCheck, HelpCircle, Save, RotateCcw
} from 'lucide-react'
...
export default function DetalleTratamientoPage() {
  ...
  // Guardado automático del odontograma
  useEffect(() => {
            ...
            <div className="flex flex-col items-center gap-16 overflow-x-auto no-scrollbar py-4">
                <div className="flex gap-8">
                  <div className="flex gap-1 border-r-2 border-slate-100 pr-8">
                    {c1.map(pid => <DienteCompleto key={pid} id={pid} datos={dentadura[pid]} itemsDiente={items.filter(i => i.diente_id === pid)} onSelect={() => setDienteSeleccionado(pid)} onCaraClick={cambiarEstadoCara} onPiezaXClick={cambiarEstadoGeneral} superior />)}
                  </div>
                  <div className="flex gap-1 pl-8">
                    {c2.map(pid => <DienteCompleto key={pid} id={pid} datos={dentadura[pid]} itemsDiente={items.filter(i => i.diente_id === pid)} onSelect={() => setDienteSeleccionado(pid)} onCaraClick={cambiarEstadoCara} onPiezaXClick={cambiarEstadoGeneral} superior />)}
                  </div>
                </div>
                <div className="flex gap-8">
                  <div className="flex gap-1 border-r-2 border-slate-100 pr-8">
                    {c3.map(pid => <DienteCompleto key={pid} id={pid} datos={dentadura[pid]} itemsDiente={items.filter(i => i.diente_id === pid)} onSelect={() => setDienteSeleccionado(pid)} onCaraClick={cambiarEstadoCara} onPiezaXClick={cambiarEstadoGeneral} invert />)}
                  </div>
                  <div className="flex gap-1 pl-8">
                    {c4.map(pid => <DienteCompleto key={pid} id={pid} datos={dentadura[pid]} itemsDiente={items.filter(i => i.diente_id === pid)} onSelect={() => setDienteSeleccionado(pid)} onCaraClick={cambiarEstadoCara} onPiezaXClick={cambiarEstadoGeneral} invert />)}
                  </div>
                </div>
            </div>
          </section>
          ...
          >
            <div className="pt-20 pb-8 px-10 bg-slate-900 text-white relative text-left">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setDienteSeleccionado(null)} className="absolute top-6 right-6 w-10 h-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-red-500 transition-all"><X size={20}/></button>
              </div>
...
function DienteCompleto({ id, datos, onSelect, onCaraClick, onPiezaXClick, superior = false, invert = false, itemsDiente = [] }: any) {
  ...
  return (
    <div className={`flex flex-col items-center gap-2 group ${invert ? 'flex-col-reverse' : ''}`}>
      ...
      {/* SECCIÓN ICONOS Y PIEZA */}
      <div className={`relative w-14 h-18 transition-all ${estadoGeneral === 'ausente' ? 'grayscale opacity-20' : 'hover:scale-110'}`}>
        <button onClick={(e) => { e.stopPropagation(); onPiezaXClick(id); }} className="absolute -top-1 -right-1 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-800 text-white p-1 rounded-full shadow-lg z-20">
          {estadoGeneral === 'ausente' ? <RotateCcw size={8}/> : <X size={8}/>}
        </button>

        <svg viewBox="0 0 100 120" className={`w-full h-full p-1 ${invert ? 'rotate-180' : ''}`}>
          <path d={getDientePath()} fill="#fff" stroke={iconos.length > 0 ? "#2563eb" : "#f1f5f9"} strokeWidth={iconos.length > 0 ? "4" : "2"} />
      ...
      {/* SECCIÓN CARAS (ODONTOGRAMA MAESTRO) */}
      <div className={`relative transition-all ${estadoGeneral === 'ausente' ? 'opacity-0 pointer-events-none' : ''}`}>
        <svg width="34" height="34" viewBox="0 0 100 100" className="cursor-pointer drop-shadow-sm">
          <path d="M10,10 L90,10 L70,30 L30,30 Z" className={getClaseCara('top')} onClick={() => onCaraClick(id, 'top')} />
          <path d="M10,90 L90,90 L70,70 L30,70 Z" className={getClaseCara('bottom')} onClick={() => onCaraClick(id, 'bottom')} />
          <path d="M10,10 L30,30 L30,70 L10,90 Z" className={getClaseCara('left')} onClick={() => onCaraClick(id, 'left')} />
          <path d="M90,10 L70,30 L70,70 L90,90 Z" className={getClaseCara('right')} onClick={() => onCaraClick(id, 'right')} />
          <path d="M30,30 L70,30 L70,70 L30,70 Z" className={getClaseCara('center')} onClick={() => onCaraClick(id, 'center')} />
        </svg>
      </div>

      {/* BOTÓN PARA SELECCIONAR (AGREGAR TRATAMIENTO) */}
      <button 
        onClick={onSelect}
        className="mt-1 bg-slate-100 hover:bg-blue-600 hover:text-white text-slate-400 p-1.5 rounded-lg transition-all"
      >
        <Plus size={12}/>
      </button>
    </div>
